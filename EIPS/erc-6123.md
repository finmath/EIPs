...
The payment can be realized by any implementation implementing an [ERC-20](./eip-20.md) token.
...
Every process state is specified; therefore, the entire post-trade process is known in advance and is deterministic over the trade's life cycle. An [ERC-20](./eip-20.md) token can be used for frictionless decentralized settlement, see reference implementation. We do provide a separate interface and implementation for a specific "Settlement Token" derived from [ERC-20](./eip-20.md).
...
A reference implementation SDC.sol is provided and is based on the [ERC-20](./eip-20.md) token standard.
